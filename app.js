...
let liveData={},liveset=new Set(),prevLiveset=new Set(),popularLiveData={};
let twitchToken=null,twitchUserToken=null;
...
const activeIframes={},twitchPlayers={};
const PARENT=location.hostname||'localhost';
let isOwner=false,globalMuted=false,globalPaused=false;
let chipDragSrc=null,nlTimer=null;

// ══════ HELPERS ══════
const isMobile=()=>window.innerWidth<=768;
const fmtV=n=>n>=1000?(n/1000).toFixed(1).replace('.0','')+'k':String(n);
const escHtml=s=>String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
const getGrade=id=>GRADES.find(g=>g.id===id)||GRADES[GRADES.length-1];
const lsGet=(k,d)=>{try{const v=localStorage.getItem(k);return v!=null?JSON.parse(v):d}catch{return d}};
const lsSet=(k,v)=>{try{localStorage.setItem(k,JSON.stringify(v))}catch{}};
const saveState=()=>lsSet('ms_sel',selected);
const saveCats=()=>lsSet('ms_cats',categories);
const saveAvatars=()=>lsSet('ms_av',avatarCache);
const getPrefs=()=>lsGet(PREFS_KEY,{});
const savePref=(k,v)=>{const p=getPrefs();p[k]=v;lsSet(PREFS_KEY,p);};
const getViewers=t=>liveData[t]?liveData[t].viewer_count:0;
const getGame=t=>liveData[t]?liveData[t].game_name:'';
const getTitle=t=>liveData[t]?liveData[t].title:'';
const getThumbnail=t=>{if(!liveData[t]||!liveData[t].thumbnail_url)return null;return liveData[t].thumbnail_url.replace('{width}','440').replace('{height}','248');};
...
function renderHistory(){
  const h=lsGet('ms_history',[]),el=document.getElementById('historyList');if(!el)return;
...
function toggleTheme(){
  ...
  const m=document.getElementById('mnavThemeIcon');if(m)m.textContent=next==='dark'?'☀️':'🌙';
}
function initTheme(){
  const saved=localStorage.getItem('ms_theme')||'dark';
  document.documentElement.setAttribute('data-theme',saved);
  const m=document.getElementById('mnavThemeIcon');if(m)m.textContent=saved==='dark'?'☀️':'🌙';
}

// ══════ TWITCH CONNECT ══════
function connectTwitchAccount(){
  window.open(`https://id.twitch.tv/oauth2/authorize?client_id=${TWITCH_CLIENT_ID}&redirect_uri=${TWITCH_REDIRECT}&response_type=token&scope=${TWITCH_SCOPES}&force_verify=true`,'_self');
}
function disconnectTwitchAccount(){
  twitchUserToken=null;
  localStorage.removeItem('ms_twitch_token');localStorage.removeItem('ms_twitch_user');
...
function checkTwitchCallback(){
  ...
  const token=params.get('access_token');if(!token)return;
  twitchUserToken=token;localStorage.setItem('ms_twitch_token',token);
  history.replaceState(null,'',window.location.pathname);
  fetch('https://api.twitch.tv/helix/users',{headers:{'Client-ID':TWITCH_CLIENT_ID,'Authorization':'Bearer '+token}})
    .then(r=>r.json()).then(d=>{
      if(d.data&&d.data[0]){const u=d.data[0];localStorage.setItem('ms_twitch_user',JSON.stringify(u));updateTwitchConnectUI(u);showToast('✅ Compte Twitch connecté : '+u.display_name);}
    }).catch(()=>{});
}
function updateTwitchConnectUI(user){
  const btn=document.getElementById('twitchConnectBtn'),info=document.getElementById('twitchAccountInfo');if(!btn||!info)return;
  if(user){
    btn.innerHTML='🔓 Déconnecter Twitch';btn.onclick=disconnectTwitchAccount;info.style.display='flex';
...
function initTwitchConnect(){
  checkTwitchCallback();
  const saved=localStorage.getItem('ms_twitch_token'),savedUser=localStorage.getItem('ms_twitch_user');
  if(saved){twitchUserToken=saved;if(savedUser){try{updateTwitchConnectUI(JSON.parse(savedUser));}catch(e){}}}
}

// ══════ AUTH ══════
function showForgotPanel(){
  document.getElementById('loginForm').style.display='none';document.getElementById('authTabsBar').style.display='none';
...
function hideForgotPanel(){
  ...
  document.getElementById('loginForm').style.display='flex';document.getElementById('authTabsBar').style.display='flex';
}
...
async function doLogin(){
  ...
  const btn=document.getElementById('loginBtn'),sp=document.getElementById('loginSpinner');btn.disabled=true;sp.classList.add('show');
...
async function doRegister(){
  ...
  const errEl=document.getElementById('registerError'),sucEl=document.getElementById('registerSuccess');errEl.classList.remove('show');sucEl.classList.remove('show');
  if(!username||!email||!pw){errEl.textContent='Remplis tous les champs.';errEl.classList.add('show');return;}
  const btn=document.getElementById('registerBtn'),sp=document.getElementById('registerSpinner');btn.disabled=true;sp.classList.add('show');
  try{
...
function listenUser(uid){
  ...
  firestoreUnsub=fb.onSnapshot(fb.doc(db,'users',uid),snap=>{
    if(snap.exists()){
      ...
      if(data.grade){
        const g=getGrade(data.grade),el=document.getElementById('gradeBadgeHeader');
        el.textContent=g.label;el.style.cssText=`background:${g.bg};border:1px solid ${g.border};color:${g.color}`;
...
async function loadGradePanel(){
  const body=document.getElementById('gradePanelBody');body.innerHTML='<div class="grade-loading">Chargement…</div>';
  ...
  try{
    ...
    statsRow.innerHTML=`<div class="grade-stat-card"><div class="grade-stat-num">${users.length}</div><div class="grade-stat-label">Membres</div></div><div class="grade-stat-card"><div class="grade-stat-num">${users.filter(u=>['vip','premium','admin'].includes(u.grade)).length}</div><div class="grade-stat-label">VIP/Admin</div></div>`;
    ...
    users.forEach(u=>{
      const g=getGrade(u.grade||'member'),name=u.displayName||u.email||u.uid.slice(0,8);
    ...
    if(!users.length){const e=document.createElement('div');e.className='grade-loading';e.textContent='Aucun utilisateur';body.appendChild(e);}
...
window._initApp=function(){
  ...
  if(!auth){document.getElementById('loadingScreen').classList.add('hidden');loadState();startApp();loadSharedSession();return;}
    ...
    if(user){
      ...
      if(isOwner){
        ...
        try{
          ...
          if(!snap.exists()||snap.data().grade!=='owner')await window._fb.setDoc(window._fb.doc(window._db,'users',user.uid),{grade:'owner',email:user.email,displayName:user.displayName||'Owner'},{merge:true});
        }catch(e){}
      }
...
async function fetchLiveStatus(){
  ...
  try{
    ...
    const r=await fetch('https://api.twitch.tv/helix/streams?'+allLogins.map(l=>'user_login='+l).join('&')+'&first=100',{headers:{'Client-ID':TWITCH_CLIENT_ID,'Authorization':'Bearer '+twitchToken}});
    ...
    (d.data||[]).forEach(s=>{
      const l=s.user_login.toLowerCase();liveData[l]=s;liveset.add(l);
...
async function fetchPopularAvatars(){
  try{
    ...
    const r=await fetch('https://api.twitch.tv/helix/users?'+needFetch.map(s=>'login='+s.twitch).join('&'),{headers:{'Client-ID':TWITCH_CLIENT_ID,'Authorization':'Bearer '+twitchToken}});
...
function renderPopularGrid(){
  ...
  sorted.forEach(p=>{
    ...
    const subHtml=isLive?`<div class="pc-live"><span class="pc-live-dot"></span>${fmtV(getViewers(p.twitch))}</div>`:`<div class="pc-sub">Hors ligne</div>`;
...
function renderSbSuggestions(){
  const pills=document.getElementById('sbSuggestionsPills'),sugg=document.getElementById('sbSuggestions');if(!pills)return;
  if(streamers.length>3){if(sugg)sugg.style.display='none';return;}
  if(sugg)sugg.style.display='';pills.innerHTML='';
  [...POPULAR_FR].sort((a,b)=>(liveset.has(b.twitch)?1:0)-(liveset.has(a.twitch)?1:0)).slice(0,10).forEach(p=>{
...
function addStreamer(r){
  ...
  render();scheduleSave();saveAvatars();showToast('✅ '+r.nom+' ajouté !');
}
...
// ══════ CATEGORIES ══════
const genCatId=()=>'cat_'+Date.now()+'_'+Math.random().toString(36).slice(2,7);
...
function createCategory(){
  ...
  categories.push({id:genCatId(),name,color:window._newCatColor||CAT_COLORS[0],members:[...(window._newCatMembers||[])]});
...
function launchCategory(ci){
  ...
  if(selected.length){launchStreams();showToast('▶ "'+cat.name+'" lancée !');}
  else showToast('Aucun streameur dans ta liste !');
}
function toggleCatExpand(header){const card=header.closest('.cat-card'),membDiv=card.querySelector('.cat-members'),btn=header.querySelector('.cat-expand-btn'),isOpen=membDiv.classList.contains('open');membDiv.classList.toggle('open',!isOpen);btn.classList.toggle('open',!isOpen);}
function toggleCatAddMember(ci,card){
  const addDiv=card.querySelector(`#cataddmember_${ci}`),membDiv=card.querySelector(`#catmembers_${ci}`);
  const isHidden=addDiv.style.display==='none';addDiv.style.display=isHidden?'block':'none';
  if(isHidden){
    membDiv.classList.add('open');card.querySelector('.cat-expand-btn').classList.add('open');
    const suggest=card.querySelector(`#catSuggest_${ci}`),cat=categories[ci];suggest.innerHTML='';
...
function addMemberToCat(ci){
  const inp=document.getElementById(`catAddInput_${ci}`);if(!inp)return;
  const val=inp.value.trim().toLowerCase();if(!val)return;
  const s=streamers.find(x=>x.twitch===val||x.nom.toLowerCase()===val);if(!s){showToast('Streameur non trouvé !');return;}
  addMemberToCatDirect(ci,s.twitch);renderCatModal();
}
...
function renderSidebar(){
  const el=document.getElementById('sbList'),cnt=document.getElementById('sbListCount');el.innerHTML='';if(cnt)cnt.textContent=streamers.length;
  ...
  if(categories.length){
    categories.forEach((cat,ci)=>{
      ...
      [...catMembers].sort((a,b)=>{const aL=liveset.has(a)?1:0,bL=liveset.has(b)?1:0;if(bL!==aL)return bL-aL;return getViewers(b)-getViewers(a);}).forEach(t=>{const s=streamers.find(x=>x.twitch===t);if(s)el.appendChild(buildItem(s));});
    });
    ...
    if(uncategorized.length){
      ...
      [...uncategorized].sort((a,b)=>{const aL=liveset.has(a.twitch)?1:0,bL=liveset.has(b.twitch)?1:0;if(bL!==aL)return bL-aL;return getViewers(b.twitch)-getViewers(a.twitch);}).forEach(s=>el.appendChild(buildItem(s)));
    }
  }else{
...
function buildItem(s){
  ...
  d.className='s-item'+(isSel?' sel':'')+(isLive?' is-live':'');
  ...
  d.innerHTML=`${avHtml}<div class="s-info"><div class="s-name">${s.nom}${isLive?' 🔴':''}</div><div class="s-sub">${s.twitch}</div></div><div class="s-right">${isLive?`<span class="s-viewers">👁 ${fmtV(viewers)}</span>`:''}<div class="${isLive?'online-dot':'offline-dot'}"></div><div class="chk">${isSel?'✓':''}</div><button class="s-edit-btn" onclick="event.stopPropagation();openEditModal('${s.twitch}')" style="display:none;background:none;border:none;cursor:pointer;font-size:.7rem;padding:2px 4px;border-radius:3px;color:var(--muted)">✏️</button><button class="s-del" onclick="event.stopPropagation();deleteStreamer('${s.twitch}')">🗑</button></div>${thumb}`;
...
function updateHeader(){
  const n=selected.length,tag=document.getElementById('limitTag');
  tag.textContent=n+'/10';tag.className='limit-tag '+(n>=8?'limit-warn':'limit-ok');
...
function updateMobileNav(){
  const nav=document.getElementById('mobileStreamsNav'),ctrl=document.getElementById('mobileStreamControls');if(!nav)return;
...
function clearAll(){if(isMobile()&&selected.length&&!confirm('Effacer tous les streams ?'))return;selected=[];Object.keys(activeIframes).forEach(id=>{activeIframes[id].src='about:blank';delete activeIframes[id];});saveState();render();if(isStreamsLaunched)endStreams();}
function launchStreams(){
  if(!selected.length)return;
  saveHistory();
  document.getElementById('welcomeScreen').style.display='none';
  document.getElementById('streamsArea').classList.add('active');
...
function endStreams(){
  document.getElementById('streamsArea').classList.remove('active');
  document.getElementById('streamsLayout').innerHTML='';
  document.getElementById('welcomeScreen').style.display='flex';
  if(fsMode)closeStreamsFsOverlay();
  isStreamsLaunched=false;updateMobileNav();
}
...
function getOrCreateIframe(id,isMain){
  ...
  if(isMobile()){
    const f=document.createElement('iframe');f.src=`https://player.twitch.tv/?channel=${id}&parent=${PARENT}&autoplay=true&muted=${!isMain}`;f.allowFullscreen=true;f.setAttribute('allow','autoplay');f.style.cssText='width:100%;height:100%;border:none';activeIframes[id]=f;return f;
  }
  const div=document.createElement('div');div.id='twitch-player-'+id;div.style.cssText='width:100%;height:100%';activeIframes[id]=div;
  setTimeout(()=>{twitchPlayers[id]=new Twitch.Player('twitch-player-'+id,{channel:id,parent:[PARENT],autoplay:true,muted:!isMain,width:'100%',height:'100%'});},100);
  return div;
}
...
function createStreamBox(id,isMain){
  ...
  ov.innerHTML=`${isMain&&selected.length>1?'<div class="main-badge">★ Principal</div>':''}<div class="stream-label">${s.nom}</div>${isLive?`<div class="stream-viewers" style="pointer-events:none"><div class="stream-viewers-dot"></div>${fmtV(v)}</div>`:''}<button class="stream-fs-btn" style="pointer-events:all" onclick="openFullscreen('${id}')">⛶</button><button class="stream-close" style="pointer-events:all" onclick="removeStream('${id}')">✕</button><button class="stream-chat-btn" style="pointer-events:all" onclick="openChatFor('${id}')">💬 Chat</button>${isMain||selected.length===1?'':`<button class="stream-promote-btn" style="pointer-events:all" onclick="promoteStream('${id}')">★ Principal</button>`}`;
  box.appendChild(getOrCreateIframe(id,isMain));box.appendChild(ov);box.addEventListener('dblclick',()=>openFullscreen(id));return box;
}
function attachVSplitter(sp,lp,cont){
  ...
  const mv=cx=>{if(!drag)return;const cw=cont.getBoundingClientRect().width,nw=Math.min(cw*.92,Math.max(cw*.08,sw+(cx-sx)));lp.style.width=nw+'px';mainPct=Math.round((nw/cw)*100);};
...
function openStreamsFsOverlay(){
  if(!selected.length)return;if(!isStreamsLaunched)launchStreams();
  fsMode=true;
...
function closeStreamsFsOverlay(){
  fsMode=false;
  const sa=document.getElementById('streamsArea'),fb=document.getElementById('fsFloatBar');
  if(sa){sa.style.position='';sa.style.inset='';sa.style.zIndex='';}if(fb)fb.style.display='none';
  const cp=document.getElementById('chatPanel'),cr=document.getElementById('chatResize');
...
function openFullscreen(id){
  ...
  document.getElementById('fsIframe').src=`https://player.twitch.tv/?channel=${id}&parent=${PARENT}&autoplay=true&muted=false`;
...
function toggleChat(){
  ...
  p.classList.toggle('hidden',!chatOpen);if(!isMobile())r.style.display=chatOpen?'block':'none';
...
(()=>{
  ...
  document.addEventListener('mousemove',e=>{if(!drag)return;p.style.width=Math.min(600,Math.max(200,sw-(e.clientX-sx)))+'px';});
...
// ══════ GLOBAL CONTROLS ══════
function toggleAllStreams(){globalPaused=!globalPaused;const btn=document.getElementById('gcPlay');if(btn){btn.textContent=globalPaused?'▶':'⏸';btn.classList.toggle('active',globalPaused);}Object.values(twitchPlayers).forEach(p=>{globalPaused?p.pause():p.play();});showToast(globalPaused?'⏸ Pause':'▶ Lecture');}
function toggleMuteAll(){globalMuted=!globalMuted;const btn=document.getElementById('gcMute');if(btn){btn.textContent=globalMuted?'🔇':'🔊';btn.classList.toggle('active',globalMuted);}Object.values(twitchPlayers).forEach(p=>p.setMuted(globalMuted));showToast(globalMuted?'🔇 Tous mutés':'🔊 Son réactivé');}
function setVolumeAll(val){const volume=parseInt(val)/100;Object.values(twitchPlayers).forEach(p=>{p.setVolume(volume);p.setMuted(volume===0);});showToast('🔈 Volume : '+val+'%');}

// ══════ KEYBOARD ══════
document.addEventListener('keydown',e=>{
  if(isMobile()||['INPUT','TEXTAREA','SELECT'].includes(e.target.tagName))return;
  switch(e.key){
    case 'Escape':if(fsMode)closeStreamsFsOverlay();else if(document.getElementById('fsOverlay').classList.contains('open'))closeFullscreen();else if(document.getElementById('catModal').classList.contains('open'))closeCatModal();else if(document.getElementById('editModal').classList.contains('open'))closeEditModal();else if(document.getElementById('gradePanel').classList.contains('open'))toggleGradePanel();break;
...
function switchSettingsTab(tab,btn){
  ...
  document.querySelectorAll('.settings-tab').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');
}
function setTheme(val){document.documentElement.setAttribute('data-theme',val);localStorage.setItem('ms_theme',val);const m=document.getElementById('mnavThemeIcon');if(m)m.textContent=val==='dark'?'☀️':'🌙';}
...
function shareSession(){
  ...
  const url=new URL(window.location.href);url.searchParams.set('streams',selected.join(','));
...
function loadSharedSession(){
  const params=new URLSearchParams(window.location.search),shared=params.get('streams');if(!shared)return;
  const ids=shared.split(',').filter(Boolean);if(!ids.length)return;
  ids.forEach(id=>{if(!streamers.find(s=>s.twitch===id))streamers.push({twitch:id,nom:id});});
  selected=ids.slice(0,10);saveState();render();
...
function sendContact(){
  const nom=document.getElementById('contactNom').value.trim(),email=document.getElementById('contactEmail').value.trim(),msg=document.getElementById('contactMsg').value.trim();
  if(!nom||!email||!msg){showToast('Remplis tous les champs !');return;}
  emailjs.send('service_8xk2oar','shnnv2m',{name:nom,email,message:msg})
    .then(()=>{document.getElementById('contactSuccess').style.display='block';['contactNom','contactEmail','contactMsg'].forEach(id=>document.getElementById(id).value='');setTimeout(()=>closeContactModal(),2000);})
