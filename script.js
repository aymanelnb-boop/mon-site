...
function launchRediff(){
  ...
  ctrlBar.innerHTML = `
    ...
    <div style="display:flex;gap:4px">${selectedVods.map((v,i)=>`<div style="padding:2px 8px;border-radius:10px;background:${i===0?'rgba(124,58,237,.25)':'rgba(255,255,255,.06)'};border:1px solid ${i===0?'var(--accent)':'var(--border)'};font-family:'Barlow Condensed',sans-serif;font-size:.65rem;font-weight:700;color:${i===0?'var(--accent2)':'var(--muted)'}">${escHtml(v.nom)}</div>`).join('')}</div>
